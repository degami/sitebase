...
/**
 * Order Shipping Methods manage
 */
class ShippingMethods extends AdminFormPage
{
    /**
     * @var string page title
     */
    protected ?string $page_title = 'Shipping Methods';
    ...
    protected function getTableItems() : array
    {
        return array_map(function($el) {
            return [
                ...
                'Class' => get_class($el),
                'Is Active' => $this->getUtils()->translate($this->getSiteData()->getConfigValue('shipping/'.$el->getCode().'/active') ? 'Yes' : 'No'),
    ...
    protected function getShippingMethods() : array
    {
        $out = array_map(function($paymentClassName) {
            ...
            return is_subclass_of($className, ShippingMethodInterface::class);
        }));
    ...
    public function getFormDefinition(FAPI\Form $form, array &$form_state): FAPI\Form
    {
        ...
        if ($type == 'config') {
            ...
            $form->addField('active', [
                'type' => 'switchbox',
                'title' => 'Is Active',
                'default_value' => App::getInstance()->getSiteData()->getConfigValue('shipping/' . $code . '/active') == 1,
            ]);

            /** @var ShippingMethodInterface $shippingMethod */
            $shippingMethod = $this->getShippingMethods()[$code];

            $form = $shippingMethod->getConfigurationForm($form, $form_state);
