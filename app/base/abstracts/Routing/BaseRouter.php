...
abstract class BaseRouter extends ContainerAwareObject implements RouterInterface 
{
    ...
    {
        if ($this->getEnvironment()->getVariable('BASE_URL') != null) {
            return $this->getEnvironment()->getVariable('BASE_URL');
        }
    ...
    public function getUrl(string $route_name, $route_params = []): string
    {
        ...
        if ($dispatcherInfo != null) {
            foreach ($route_params as $var_name => $value) {
                ...
                $dispatcherInfo['path'] = preg_replace($regexp, (string) $value, $dispatcherInfo['path']);
            }
